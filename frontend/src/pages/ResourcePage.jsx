...
const configs = {
  ...
  staff: {
    ...
    fields: [
      ...
      { name: 'email', label: 'Email' },
      { name: 'phone', label: 'Phone' },
      { name: 'role', label: 'Role', type: 'select', options: ['Super Admin', 'Institute Admin', 'Teacher', 'Student'].map((item) => ({ label: item, value: item })) },
      { name: 'centerId', label: 'Center', type: 'select', source: 'centers', optionLabel: 'centerName' }
    ]
  },
  activities: {
...
export default function ResourcePage({ resource }) {
  ...
  useEffect(() => {
    for (const key of ['centers', 'cohorts', 'students']) {
  ...
  return (
    <div>
      ...
      <DataTable columns={config.columns} rows={state.items} loading={state.loading} onEdit={(row) => { setEditing(row); setOpen(true); }} onDelete={remove} />
